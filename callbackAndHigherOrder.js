...
const uniq = (arr, cb) => {
  ...
  for (let i = 0; i < joe.length; i++) {
    ...
    for (let j = 0; j < joe.length; j++) {
      // if its not the same index
      if (i !== j) {
        // we compare each bit of the array to each other
        if (joe[i] === joe[j]) {
          joe.splice(i, 1);
          break;
        }
...
each(names, (name, num) => {
  console.log(`The item at index ${num} is ${name}`);
});
...
const addSevenHundredEightyFourThousandEightHundredAndSeventyTwo = addingFactory(784872);

console.log(addSevenHundredEightyFourThousandEightHundredAndSeventyTwo(1));